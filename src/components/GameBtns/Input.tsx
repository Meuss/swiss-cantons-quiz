...
import { normalizeDiacritics, replaceSpecialChars } from '../../utils/specialChars';
...
const Input = () => {
  ...
  const gameStarted = useSelector(
    (state: RootState) => state.game.gameStatus === 'running' || state.game.gameStatus === 'ended'
  );
  ...
  const remainingCantons = useSelector((state: RootState) => state.remainingCantons);
  ...
  useEffect(() => {
    const normalizedInput = normalizeDiacritics(replaceSpecialChars(input.toLowerCase()));

    const matchFound = Object.values(remainingCantons).some((translations) =>
      translations
        .map((translation) => normalizeDiacritics(replaceSpecialChars(translation.toLowerCase())))
